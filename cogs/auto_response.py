...
class Ar(commands.Cog, name="Auto Responce", description="Easy way to add Auto Responce"):
    ...
    async def on_message(self, message):
        if message.author.bot or self.heist_ar_active == False or message.guild.id != 785839283847954433 and message.channel.id == 855717169584537660:
            return
    ...
    async def set_heist_ar(self, interaction: Interaction, time: str, channel: discord.TextChannel, amount: str, role: discord.Role=None, remove_old: bool=False):
        ...
        self.heist_ar = {'_id': interaction.user.id, 'time': time, 'channel': channel.id, 'ammount': amount, 'role': role.id if role else None, "expir": times}
