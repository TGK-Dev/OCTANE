...
class Invite_Panel(nextcord.ui.View):
    def __init__(self, staff_invite, public_invite):
        super().__init__(timeout=3600)
        self.staff_invite = staff_invite
        self.public_invite = public_invite

    @nextcord.ui.button(label="Staff-invite", style=nextcord.ButtonStyle.red, custom_id="staff-invite")
    async def staff_invite(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
    ...
    @nextcord.ui.button(label="Public Invite", style=nextcord.ButtonStyle.green, custom_id="public-invite")
    async def public_invite(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
...
class Cog_name(commands.Cog, description="Ban Battle Event Module"): 
    ...
    async def setup(self, ctx):
        ...
        overwrites = {guild.default_role: nextcord.PermissionOverwrite(read_messages=False, view_channel=False)}
        update = {guild.default_role: nextcord.PermissionOverwrite(read_messages=True, send_messages=False)}
        ...
        embed = nextcord.Embed(title="Ban Battle", description=f"ban battle server has been created by {ctx.author.mention}\nThere are two invite links ones", color=0x0af4f1)
    ...
    @commands.check_any(checks.can_use())
    async def takeover(self, ctx, guild: nextcord.Guild):
    ...
    async def on_member_join(self, member):
        ...
        if member.guild.name == "Ban Battle":
            ...
            role = nextcord.utils.get(member.guild.roles, name="TG Staff")
    ...
    async def ban(self, ctx, member: nextcord.Member):
        if ctx.guild.name == "Ban Battle":
            role = nextcord.utils.get(ctx.guild.roles, name="TG Staff")
