...
class Event(commands.Cog,  description=description):
            ...
            if trinnger1 > trinnger2:
                ...
                embed = discord.Embed(
					...
					***Hint***: `{word[hint]}`",
                    color=0x2ECC71)
                embed.set_footer(
                    text="Still In Ealry Stage This Might Get Change")
    ...
    async def score(self, ctx, member: discord.Member = None):
        ...
        if data is None:
            embed = discord.Embed(
                description="User Score Is not Found Maybe he never won who know?", color=0xE67E22)
            return await ctx.send(embed=embed)
        #score = data["score"]
        embed = discord.Embed(
            description=f"The user {member.mention} Has Total: {data['score']}", color=0x2ECC71)
        await ctx.send(embed=embed)
