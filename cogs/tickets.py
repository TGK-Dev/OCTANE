...
class tickets(commands.Cog):
    ...
    @commands.command(name="close", description="close The tickets", usage="")
    async def close(self, ctx):
        if ctx.channel.category.id == 829230513516445736:
            if ctx.channel.permissions_synced:
                await ctx.send("tickets Is Closed already")
            else:
    ...
    @commands.command(name="delete", description="delete the tickets", usage="")
